...
export default function EditUser(){
  //khai báo gtri khởi tạo
    ...
    useEffect(async () => {
      ...
      //có sự thay đổi, phương thức onChangeInput sẽ được gọi để cập nhật state 
...
// gửi hình ảnh qua API hoặc lưu trữ trong cơ sở dữ liệu dưới dạng chuỗi
      const handleOnChangeImage = async (event) => {
        let data = event.target.files;
        let file = data[0];//lấy file đầu tiên  
        if (file) {
           // cove kiểu ảnh thành chuỗi base64 để lưu trữ 
          let base64 = await CommonUtils.getBase64(file);
           //tạo url tạm thời để hiển thị hình ảnh
          let objectUrl = URL.createObjectURL(file);
          //lưu hình ảnh vào state
          setPreviewImgURL(objectUrl)
          setAvatar(base64)// Lưu base64 vào state
        }
      };
      ...
      const handleSaveUser = () => {
        ...
        //Nếu hợp lệ, dispatch action để cập nhật thông tin người dùng
...
// sự kiện onChange sẽ được kích hoạt và
//  gọi hàm handleOnChangeImage.