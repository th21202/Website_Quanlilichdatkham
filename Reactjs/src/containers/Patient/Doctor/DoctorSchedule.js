      ...
      <>
        <div className="doctor-schedule-container">
          <div className="all-schedule">
            ...
            <span className="day-selector-title">Booking slots</span>
            <div className="day-selector">
            {allDays && allDays.length > 0 &&
              allDays.map((item, index) => {
                const isSelected = item.value === this.state.today;
                return (
                  <button
                    key={index}
                      className={`day-button ${isSelected ? 'active' : ''}`}
                    onClick={() => this.handleOnChangeSelect({ target: { value: item.value } })}
                  >
                <div className="day-name">
                  {language === LANGUAGES.VI 
                  ? moment(item.value).locale('vi').format('ddd').toUpperCase()
                        : moment(item.value).locale('en').format('ddd').toUpperCase()
                      }
                  </div>
