  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!inputId || !location) {
        ...
        if (data) {
          //tìm kiếm doctor theo specialtyId
          let doctorSpecialty = [];
          //nếu location là ALL thì tìm kiếm doctor theo specialtyId
          if (location === "ALL") {
              ...
              include: [
                {
                  ...
                  attributes: ["id"],
                  where: { 
                    id: db.Sequelize.col('Doctor_Infor.doctorId'),
                    status: 0 // Chỉ lấy các bác sĩ còn active
                  }
                }
              ],
              raw: true,
